...
void CE32_HJ_RXISR(struct CE32_HJ580_Handle *handle)
{
	...
	if(UART_CHECK_RXNE!=0)
	{
		...
		{
			dataMGR_enQueue_byte(&handle->RX_MGR,HJ_UART_DR&0xff); //if buffer not empty,read one byte to RX buffer
		}
	...
	{		
		if(UART_CHECK_TXE!=0)
		{
			if(handle->TX_MGR.bufferUsed[0]>0){
				HJ_UART_DR=dataMGR_deQueue_byte(&handle->TX_MGR,0); //if buffer not empty, send one byte from TX buffer
			}
			else
			{
...
void CE32_UART_TRANSMIT_DMA(uint8_t *data,uint16_t size,int16_t sig1,int32_t sig2){
	UART_DMA_Disable();
	while(UART_CHECK_TXE==0){};
	HJ_UART_DR = 0xAA; //send header (AA 55 are charge balanced)
	while(UART_CHECK_TXE==0){};
	HJ_UART_DR = 0xAA; //send header (AA 55 are charge balanced)
	while(UART_CHECK_TXE==0){};
	HJ_UART_DR = 0xAA; //send header (AA 55 are charge balanced)
	...
	HJ_UART_DR = 0xAA; //send header (AA 55 are charge balanced)
	while(UART_CHECK_TXE==0){};
	HJ_UART_DR = 0x55; //send header
#ifndef MANCHESTER
	while(UART_CHECK_TXE==0){};
	HJ_UART_DR = (sig1&0x00ff); 			//send header
	while(UART_CHECK_TXE==0){};
	HJ_UART_DR = (sig1&0xff00)>>8; //send header
	while(UART_CHECK_TXE==0){};
	HJ_UART_DR = (sig2&0x000000ff); 			//send header	
  while(UART_CHECK_TXE==0){};
	HJ_UART_DR = (sig2&0x0000ff00)>>8; //send header
	while(UART_CHECK_TXE==0){};
	HJ_UART_DR = (sig2&0x00ff0000)>>16; 			//send header
	while(UART_CHECK_TXE==0){};
	HJ_UART_DR = (sig2&0xff000000)>>24; //send header
#else
	uint32_t temp;
	temp=Manchester_converter_Short(sig1);
	while(UART_CHECK_TXE==0){};
	HJ_UART_DR = (temp&0x00ff); 			//send header
	while(UART_CHECK_TXE==0){};
	HJ_UART_DR = (temp&0xff00)>>8; //send header
	while(UART_CHECK_TXE==0){};
	HJ_UART_DR = (temp&0x00ff0000)>>16; 			//send header
	while(UART_CHECK_TXE==0){};
	HJ_UART_DR = (temp&0xff000000)>>24; //send header
	temp=Manchester_converter_Short(sig2&0x0000ffff);
	while(UART_CHECK_TXE==0){};
	HJ_UART_DR = (temp&0x000000ff); 			//send header	
  while(UART_CHECK_TXE==0){};
	HJ_UART_DR = (temp&0x0000ff00)>>8; //send header
	while(UART_CHECK_TXE==0){};
	HJ_UART_DR = (temp&0x00ff0000)>>16; 			//send header
	while(UART_CHECK_TXE==0){};
	HJ_UART_DR = (temp&0xff000000)>>24; //send header
  ...
  while(UART_CHECK_TXE==0){};
	...
	HJ_UART_DR = (temp&0xff000000)>>24; //send header
#endif
	HJ_DMA_TX_MAR=(uint32_t)data;
	HJ_DMA_TX_NDTR=size;
	HJ_DMA_TX_CR|=DMA_IT_TC;
	UART_DMA_Enable();
}
